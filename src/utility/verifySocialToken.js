...
module.exports = (socialId, socialToken) => {
  return new Promise(async (resolve, reject) => {
    console.log('at token', socialToken);
    let socialAuth = await axios.get(
      `https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=${socialToken}`
    );
    ...
    if (socialAuth.sub != socialId) {
      reject(
        new errors.BadRequest('Social Token does not match Social Id.', {
          socialToken,
          socialId
        })
      );
